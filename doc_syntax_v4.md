...
* `<y` = “this depends on something earlier (id y)”
* `>y` = “this depends on something that comes later (id y)”
  The “something” is usually the subject/verb (or hosting unit) you conceptually attach to. 
...
`[CPx … ]` are used for **additional coordinated predicates** (and sometimes other “parallel constituents” when you decide they behave like a series). They:

* **Do not** add indentation level
* Must open on their own line with `[CPx …`
* Close with `]` on its own line
* Live “inside” the current clause level 
...
A clause that “hangs off” another unit (IC, another DC, or sometimes a CP predicate), marked with `<y` or `>y`.
...
Use PP when the participial phrase is **removable without breaking syntax**, and/or **easily rewritable as a dependent clause** (“who/that …”).
Do **not** tag it as PP if it is syntactically obligatory (cleft-like focus) or if it’s acting as a noun (“Making him happy…”). 

### AP — Adjunct Phrase (non-PP), optional

Use AP for other **optional adjunct bundles** that behave like “add-ons” (e.g., certain purpose infinitives, postposed descriptive bundles, afterthought modifiers).
...
You’ll be reusing every character (including punctuation). Don’t “clean it up.”
...
### Step 2 — Create the first top-level block and assign id “1”
...
### Step 3 — Walk left-to-right and “cut in” embedded units
...
* The relative clause “of whose…” is nested as DC2<1 inside IC1
...
* The optional participial “speaking…” becomes PP4 nested under the relevant action
...
* **Do not add or remove spaces.** The analysis is conceptually “inserted between” original characters.
* One tolerated convenience: the **first tag line** that carries text may have a “non-significant” extra space for clarity (meant to be removed later by tooling). 
* Coordination words like **and / but** are bundled with the clause or phrase they introduce, even if it’s slightly debatable. 
...
* **CP uses brackets**, not parentheses, and doesn’t add indentation
* **Punctuation and spacing are preserved**

### Pitfalls worth insisting on (because they’re easy to miss)

1. **Losing or duplicating words** when you “cut in” nested units. If you’re unsure, rebuild by reading the analysis top-to-bottom and verifying it reproduces the sentence perfectly.
